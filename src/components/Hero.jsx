...
import cvFile from '../assets/Kajoornan_Selvanathan_CV.pdf'

export default function Hero() {
  const typed = useTypewriter(['Problem Solver', 'Data Science Enthusiast','AI Developer'], 100, 1600)

  return (
    <section id="home" className="relative min-h-fit flex items-start justify-center overflow-hidden pt-16 md:pt-20 lg:pt-24 pb-32 md:pb-40">
      ...
      <div className="max-w-7xl mx-auto px-6 w-full grid md:grid-cols-2 gap-10 items-start relative z-10">
        {/* Left Content */}
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, ease: 'easeOut' }}>
          <p className="text-sm uppercase tracking-[0.35em] text-cyan-400 mb-4">Hello 👋 , I'm</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <motion.a href={cvFile} target="_blank" rel="noreferrer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-3 rounded-full border border-cyan-500/50 text-cyan-400 font-semibold hover:bg-cyan-500/10 transition-all text-center">
              View CV
            </motion.a>
          </div>
        </motion.div>
        ...
        {/* Right - Profile Image */}
        <motion.div initial={{ opacity: 0, x: 40, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.9, ease: 'easeOut', delay: 0.1 }} className="relative flex justify-center md:justify-end md:mt-0 lg:mt-0">
